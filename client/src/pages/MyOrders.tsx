...
export const MyOrders: React.FC = () => {
  ...
  usePageTitle({
    title: "Tra cứu đơn hàng - SPTN Perfume",
    description:
      "Tra cứu và xem chi tiết các đơn hàng của bạn bằng email đã sử dụng khi đặt hàng.",
    image:
  ...
  const fetchOrders = useCallback(
    async (email?: string) => {
      ...
      if (isAuthenticated && !email) {
        console.log(
          "[MyOrders] 🔵 Fetching orders for authenticated user (by userId)"
        );
        setIsLoading(true);
        setError(null);
        setHasSearched(true);
      ...
      try {
        const url = `/orders/my-orders?email=${encodeURIComponent(
          emailToSearch.trim()
        )}`;
        console.log("[MyOrders] 🔵 API Request URL:", url);
        ...
        if (response && response.length === 0) {
          console.log(
            "[MyOrders] ℹ️ No orders found for email:",
            emailToSearch
          );
      ...
      orders.forEach((order) => {
        if (
          ...
          (order.payment?.method === "QR_CODE" ||
            order.payment?.method === "E_WALLET" ||
            order.payment?.method === "QR_PAYMENT")
        ) {
          ...
          if (remaining > 0) {
            ...
            console.log(
              "[MyOrders] ⏰ Timeout reached for order:",
              order.orderId
            );
            apiService
              .post(`/orders/${order.orderId}/cancel-timeout`, {})
              .then((response) => {
                console.log(
                  "[MyOrders] 🔵 Timeout check response for order",
                  order.orderId,
                  ":",
                  response
                );
              ...
              .catch((err) => {
                console.error(
                  "[MyOrders] ❌ Error checking timeout for order",
                  order.orderId,
                  ":",
                  err
                );
