...
interface ProductsFilterContextType extends ProductsFilterState {
  ...
  setAppliedCategories: (categories: number[] | ((prev: number[]) => number[])) => void;
  setAppliedSearchQuery: (query: string) => void;
  setAppliedPriceRange: (range: [number, number] | null | ((prev: [number, number] | null) => [number, number] | null)) => void;
  setSortBy: (sortBy: string) => void;
  
  // Setters for temporary filters
  setSelectedBrands: (brands: number[] | ((prev: number[]) => number[])) => void;
  setSelectedCategories: (categories: number[] | ((prev: number[]) => number[])) => void;
  setSearchQuery: (query: string) => void;
  setPriceRange: (range: [number, number] | null | ((prev: [number, number] | null) => [number, number] | null)) => void;
...
export const ProductsFilterProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Save to storage whenever applied filters change
  useEffect(() => {
  ...
  }, [appliedBrands, appliedCategories, appliedSearchQuery, appliedPriceRange, sortBy, currentPage]);
  ...
  const resetFilters = useCallback((priceBounds?: [number, number] | null) => {
    ...
    if (priceBounds !== null && priceBounds !== undefined && priceBounds.length === 2) {
    ...
    // Reset to first page
    setCurrentPage(0);
  }, [setSelectedBrand, setSelectedBrands, setAppliedBrands, setSelectedCategory, setSelectedCategories, setAppliedCategories, setSearchQuery, setAppliedSearchQuery, setBrandSearchQuery, setPriceRange, setAppliedPriceRange, setSortBy, setCurrentPage]);
  ...
  const hasActiveFilters = useMemo(() => {
    return appliedBrands.length > 0 ||
      ...
      appliedSearchQuery !== "" ||
      sortBy !== "newest"; // "newest" is the default, so it's not considered an active filter
    // Note: appliedPriceRange check is done in Products component where priceBounds is available
  }, [appliedBrands.length, appliedCategories.length, appliedSearchQuery, sortBy]);
  
  // Check if there are pending filter changes
  const hasPendingFilters = useMemo(() => {
    return JSON.stringify(selectedBrands.sort()) !== JSON.stringify(appliedBrands.sort()) ||
      ...
      (priceRange !== null && appliedPriceRange === null);
  }, [selectedBrands, appliedBrands, selectedCategories, appliedCategories, searchQuery, appliedSearchQuery, priceRange, appliedPriceRange]);
